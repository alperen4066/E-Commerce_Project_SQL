...
SELECT COUNT(distinct Cust_id) count_cust
from combined_table
...
SELECT distinct Cust_id, Order_Date,FIRST_ORDER_DATE,DATEDIFF(day,FIRST_ORDER_DATE,Order_Date) as diff_date 
FROM TAB1
where row_num = 3    
...
SELECT Cust_id,sum_prod11,sum_prod14,
		FORMAT(1.0*sum_prod11/sum_prod,'N2') AS ratıo_p11,	
		--CAST (1.0*sum_prod11/ sum_prod AS NUMERIC (3,2)) AS ratıo_p11,
		FORMAT(1.0*sum_prod14/sum_prod,'N2') as ratıo_p14
